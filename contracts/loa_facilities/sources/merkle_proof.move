...
module loa_facilities::merkle_proof {
    // use std::hash;
    use std::vector;
    use sui::hash;

    use loa_facilities::vectors;
    //use std::debug;
    ...
    fun process_multi_proof(
        ...
        assert!(leaves_len + proof_len - 1 == total_hashes, EINVALID_MULTI_PROOF);
        ...
        if (total_hashes > 0) {
            // Check proof validity.
            assert!(proof_pos == proof_len, EINVALID_MULTI_PROOF);
